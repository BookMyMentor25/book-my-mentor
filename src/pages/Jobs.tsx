...
const Jobs = () => {
  ...
  const getCompanyWebsite = (job: any) => {
    return (job.recruiters as any)?.website || job.company_website;
  };
  ...
  const formatSalary = (min?: number, max?: number, currency = 'INR', salaryPeriod?: string) => {
    ...
    const periodMap: Record<string, string> = {
      'per_month': '/month', 'per_annum': '/year', 'monthly': '/month', 'yearly': '/year',
      'hourly': '/hour', 'daily': '/day', 'weekly': '/week', 'contract': ' (contract)',
    };
    const periodLabel = salaryPeriod ? (periodMap[salaryPeriod] || `/${salaryPeriod}`) : '';
    const symbol = currency === 'INR' ? '₹' : '$';
    if (min && max) return `${symbol}${formatter.format(min)} - ${formatter.format(max)}${periodLabel}`;
            ...
            {isLoading ? (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {jobs.map((job) => {
                  ...
                  return (
                    <Card key={job.id} className="h-full hover:shadow-lg hover:border-primary/30 transition-all duration-300 group cursor-pointer relative">
                      {/* Share button */}
                      <div className="absolute top-3 right-3 z-10" onClick={(e) => e.stopPropagation()}>
                        <ShareJobButton jobId={job.id} jobTitle={job.title} companyName={companyName} />
                      </div>
